 ...
 * 第四步：使用Bean
 * 第五步：注销Bean（会调用Bean的destroy方法，注意：这个destroy方法需要自己写自己配）
 *
 * @author baimiaomiao
 * @version 1.0
 * @className User
...
public class User implements BeanNameAware, BeanClassLoaderAware, BeanFactoryAware, InitializingBean,DisposableBean {
    ...
    public void afterPropertiesSet() throws Exception {
        System.out.println("InitializingBean's afterPropertiesSet method execute!");
    }

    @Override
    public void destroy() throws Exception {
        System.out.println("DisposableBean's destroy method execute!");
    }
}
